...
// Function to encode a Seedling function to NASM
...
void encode_declare_section(const char* ident, int type, enum scope_type scope) {
    ...
    fprintf(nasm_out, "; Declare section: %s\n", ident);
}
...
void encode_universal_scope_section(FILE* output, const char* label, enum scope_type scope) {
    if (output == NULL) {
        fprintf(stderr, "Error: output is NULL in encode_universal_scope_section\n");
        return; // Check for null pointer
    }
    // Generate NASM code for the universal scope section
    fprintf(output, "section .%s\n", label);
}
